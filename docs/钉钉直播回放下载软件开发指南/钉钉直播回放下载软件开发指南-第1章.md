...
Cookie管理器负责管理钉钉直播回放下载所需的Cookie,包括Cookie的读取、保存、删除等功能。
...
在`config/settings.yaml`中配置Cookie相关设置:
...
URL解析器负责解析钉钉直播回放链接,提取视频ID、认证参数等信息。
...
在`config/settings.yaml`中配置URL相关设置:
...
鉴权管理器负责管理钉钉直播回放的鉴权信息,包括Cookie、认证参数等。
